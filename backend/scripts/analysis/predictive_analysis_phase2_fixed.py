#!/usr/bin/env python3
"""
Blue Data Analysis - Phase 2: Predictive Modeling (Fixed)
...
class BlueDataPredictor:
    ...
    def predict_logistics_optimization(self):
        ...
        y_pred_final = best_model.predict(X_test_scaled if best_model in [models['Random Forest'], models['Gradient Boosting']] else X_test)
    ...
    def predict_customer_behavior(self):
        ...
        # Generate predictions
        y_pred_final = best_model.predict(X_test_scaled if best_model == models['Random Forest'] else X_test)
    ...
    def predict_market_segmentation(self):
        ...
        # If no specific features, use numeric columns and encoded categorical columns
        if not segmentation_features:
            numeric_cols = self.df_processed.select_dtypes(include=[np.number]).columns.tolist()
            encoded_cols = [col for col in self.df_processed.columns if '_encoded' in col]
            segmentation_features = numeric_cols + encoded_cols[:5]
        
        # Ensure we have at least 2 features for PCA
        if len(segmentation_features) < 2:
            all_numeric = self.df_processed.select_dtypes(include=[np.number]).columns.tolist()
            segmentation_features = all_numeric[:5]
        
        print(f"📊 Segmentation Features: {segmentation_features}")
        
        # Prepare data - handle categorical columns properly
        X = self.df_processed[segmentation_features].copy()
        ...
        # Analyze clusters - use numeric features for analysis
        numeric_features = self.df_processed.select_dtypes(include=[np.number]).columns.tolist()
        cluster_analysis = self.df_processed.groupby('market_segment')[numeric_features].mean()
...
def main():
    ...
    print("🤖 BLUE DATA PREDICTIVE MODELING (Fixed)")
    print("="*50)
